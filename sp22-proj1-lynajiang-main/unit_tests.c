...
bool test_is_tail() {
  ...
  if (!assert_true("test w", is_tail('w') == true)) {
    return false;
  }
  if (!assert_true("test q", is_tail('q') == false)) {
    return false;
  }
  return true;
}
...
bool test_is_snake() {
  ...
  if (!assert_true("test ^", is_snake('^') == true)) {
    return false;
  }
  if (!assert_true("test >", is_snake('>') == true)) {
    return false;
  }
  if (!assert_true("test non-char", is_snake('b') == false)) {
    return false;
  }
  return true;
}
...
bool test_body_to_tail() {
  if (!assert_equals_char("test w", body_to_tail('^'), 'w')) {
    return false;
  }

  if (!assert_equals_char("test d", body_to_tail('>'), 'd')) {
    return false;
  }
...
bool test_incr_x() {
  if (!assert_equals_int("test >", incr_x('>'), 1)) {
    return false;
  }
  if (!assert_equals_int("test d", incr_x('d'), 1)) {
    return false;
  }
  if (!assert_equals_int("test a", incr_x('a'), -1)) {
    return false;
  }
  if (!assert_equals_int("test <", incr_x('<'), -1)) {
    return false;
...
bool test_incr_y() {
  if (!assert_equals_int("test v", incr_y('v'), 1)) {
    return false;
  }
  if (!assert_equals_int("test s", incr_y('s'), 1)) {
    return false;
  }
  if (!assert_equals_int("test w", incr_y('w'), -1)) {
    return false;
  }
  if (!assert_equals_int("test ^", incr_y('^'), -1)) {
    return false;
  }
  return true;
}
